...
| REQ-IMG-2 | The image runs as the unprivileged `nobody` user (UID `65534`) and exposes port `10019`. It also ships an empty `/cache` directory owned by that user — the default `pbs.cache-fs-path` (`REQ-CFG-11`) — so a volume mounted there is writable by the exporter. No volume is declared: the `fs` cache stays opt-in. | `Dockerfile` declares `USER 65534` and `EXPOSE 10019`; `docker run --rm pbs-exporter-node --version` prints the version line; `docker run --rm --entrypoint ls pbs-exporter-node -ld /cache` shows it owned by `65534`. **[offline-ok]** |
