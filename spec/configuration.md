...
| REQ-CFG-2 | Environment variables `PBS_ENDPOINT`, `PBS_USERNAME`, `PBS_API_TOKEN`, `PBS_API_TOKEN_NAME`, `PBS_TIMEOUT`, `PBS_INSECURE`, `PBS_SNAPSHOTS_CACHE`, `PBS_CACHE_DRIVER`, `PBS_CACHE_FS_PATH`, `PBS_METRICS_PATH`, `PBS_LISTEN_ADDRESS`, `PBS_LOGLEVEL`, `PBS_LOGFORMAT` override flags/defaults. | Set `PBS_METRICS_PATH=/x` while passing `--pbs.metrics-path=/y`; metrics MUST be at `/x`. **[offline-ok]** |
| REQ-CFG-3 | Defaults: username `root@pam`, api token name `pbs-exporter`, timeout `5s`, insecure `false`, snapshots cache `false`, cache driver `memory`, cache fs path `/cache`, metrics path `/metrics`, listen address `:10019`, loglevel `info`, logformat `text`. | Start with no config; logs report listen `:10019`, path `/metrics`. **[offline-ok]** |
...
| REQ-CFG-9 | `pbs.snapshots.cache` is a boolean (same string parsing as `pbs.insecure`: `1`/`t`/`true` / `0`/`f`/`false`) that enables serving stale `pbs_snapshot_*` metrics from a cache when a scrape fails; invalid (non-boolean) values cause a fatal error log and non-zero exit. See `REQ-SCRAPE-6` for the behaviour and `REQ-CFG-11` for where the cache is stored. | `PBS_SNAPSHOTS_CACHE=true node src/run.ts` starts; `PBS_SNAPSHOTS_CACHE=maybe` exits non-zero with an `ERROR:` line. **[offline-ok]** |
...
| REQ-CFG-11 | `pbs.cache-driver` selects where the snapshot cache (`REQ-CFG-9`) is stored: `memory` (default — a per-process store, lost on restart) or `fs` (a JSON document in the directory given by `pbs.cache-fs-path`, default `/cache`, so the cache survives a container restart — see `REQ-SCRAPE-7`). Any other driver name is rejected with an error naming the accepted set; with `fs` selected, an empty `pbs.cache-fs-path` is rejected as well. The path is only used by the `fs` driver and is not touched (no directory is created) when the driver is `memory` or when `pbs.snapshots.cache` is disabled. | `PBS_CACHE_DRIVER=redis node src/run.ts` exits non-zero with an `ERROR:` line listing `memory, fs`; `PBS_SNAPSHOTS_CACHE=true PBS_CACHE_DRIVER=fs PBS_CACHE_FS_PATH=/tmp/pbs-cache node src/run.ts` starts and logs the fs driver and its path. **[offline-ok]** |
