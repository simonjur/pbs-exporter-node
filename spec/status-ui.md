...
| REQ-UI-7 | The page renders an app-level footer showing the build the exporter was built from: the exporter `version` and the build `commit` hash (from `/api/status` `exporter.commit`, i.e. `PBS_BUILD_COMMIT`), plus a link to the metrics path and the last-refresh time. | Load `/`; the footer shows `commit <hash>` matching `/api/status`'s `exporter.commit`. **[offline-ok]** |
