...
| REQ-UI-7 | The page renders an app-level footer showing the build the exporter was built from: the exporter `version` and the build `commit` hash (from `/api/status` `exporter.commit`, i.e. `PBS_BUILD_COMMIT`), plus a link to the metrics path and the last-refresh time. The footer is a compact strip sized to its content — it must not stretch to absorb the leftover viewport height (Vuetify's `.v-footer` defaults to `flex: 1 1 auto`, so it is given `flex-grow-0`); the main content area takes the free space and the footer sits directly below it, at the bottom of the viewport when the page is short. | Load `/`; the footer shows `commit <hash>` matching `/api/status`'s `exporter.commit`, is one line tall (well under 80px for a short page), and its bottom edge meets the bottom of the viewport. **[offline-ok]** |
| REQ-UI-8 | The app bar has a light/dark theme toggle. The chosen theme is persisted in `localStorage` (key `pbs-exporter-theme`, value `light`/`dark`) and restored on the next load; with no stored preference the page follows the OS `prefers-color-scheme`. | Load `/`, toggle to dark, reload → page stays dark and `localStorage['pbs-exporter-theme'] === 'dark'`; clear the key with the OS in dark mode → page loads dark. **[offline-ok]** |
